create or replace procedure si2spt943tv2(p IN varchar2,sdate IN varchar2,edate IN varchar2)
AUTHID CURRENT_USER
IS
...
--            NVL(TO_CHAR(trunc(c.q2,6)),'-') as "W2",                  -- 12 W2 (W1 и W2 не отображаются)
            ...
            /*'-' as "P1гв",*/ NVL(TO_CHAR(trunc(c.p3,2)),'-'),           -- 18 P1гвс (ТВ2) -> p3 должно быть p4 28.11.2013
--            '-' as "W1гв", --NVL(TO_CHAR(trunc(c.q4,2)),'-'),         -- 19 W1гвс (ТВ2) 
            NVL(TO_CHAR(trunc(c.q4,6)),'-'),                            -- 19 W1гвс (ТВ2) 
            /* ТВ2 завершение --------------------------------------------------------------- */
            '-' as "tхв", --NVL(TO_CHAR(trunc(c.tcool,2)),'-'),         -- 20 TCool -> tхв не измеряется
            '-' as "Pхв", --NVL(TO_CHAR(trunc(c.pxb,2)),'-'),           -- 21 PXB -> Pxv не измеряется
...
--            NVL(TO_CHAR(c.worktime),'-'), -- 23 Tp
            ...
            NVL(TO_CHAR(c.errtime),'-'),                                -- 25 Tog (время отключения горячей воды)
