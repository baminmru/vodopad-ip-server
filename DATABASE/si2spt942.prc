create or replace procedure si2spt942(p IN varchar2,sdate IN varchar2,edate IN varchar2)
AUTHID CURRENT_USER
IS
...
-- 20.01.16 '-', --NVL(TO_CHAR(trunc(c.v1,2)),'-'), -- 4 данные отсутствуют ???? данные есть!
...
-- 20.01.16 '-', --NVL(TO_CHAR(trunc(V2,2)),'-'), -- 9 данные отсутствуют!!!!!!!!!!!!!!!!!!
...
--            '-', --NVL(TO_CHAR(trunc(c.P2,2)),'-'), -- 11 данные отсутствуют
            ...
            NVL(TO_CHAR(trunc(c.q1,6)),'-'),  -- 14 dW тепловая энергия (добавил 07.04.2016 )
            --'-', --NVL(TO_CHAR((trunc(c.q1,6)-trunc(c.q2,6))),'-'), -- as "dW12", -- 14 dW12
-- 20.01.16 '-', --NVL(TO_CHAR(trunc(c.m4,2)),'-'),-- 15 M1гвс? данные отсутствуют?
            NVL(TO_CHAR(trunc(c.m4,6)),'-'),-- 15 M1гвс? данные отсутствуют?
            NVL(TO_CHAR(trunc(c.v4,6)),'-'), -- 16 V1гвс??????
-- 20.01.16 '-',--NVL(TO_CHAR(trunc(c.t4,2)),'-'), -- 17 T1гвс данные отсутствуют!!!
            NVL(TO_CHAR(trunc(c.t4,6)),'-'), -- 17 T1гвс данные отсутствуют!!!
            '-', --NVL(TO_CHAR(trunc(c.p4,2)),'-'), -- 18 P1гвс ????????????
            NVL(TO_CHAR(trunc(c.q2,6)),'-'), -- 19 W1гвс ????????????
