create or replace procedure si2spt943(p IN varchar2,sdate IN varchar2,edate IN varchar2)
AUTHID CURRENT_USER
IS
...
OPEN cdat FOR
     select
            ...
            NVL(TO_CHAR(trunc(c.P1,6)),'-'), -- 6 данные отсутствуют -- 20.06.2008 добавил данные
...
--            NVL(TO_CHAR((trunc(c.q1,6)-trunc(c.q2,6))),'-'), -- as "dW12", -- 14 dW12
            NVL(TO_CHAR((trunc(c.q1,6))),'-'), -- as "dW12", -- 14 dW12
            NVL(TO_CHAR(trunc(c.m3,6)),'-'), -- 15 M1гвс 2009.04.15 OK!
            NVL(TO_CHAR(trunc(c.v3,6)),'-'), -- 16 V1гвс??????
            NVL(TO_CHAR(trunc(c.t3,6)),'-'), -- 17 T1гвс
