create or replace procedure si2mt200(p IN varchar2,sdate IN varchar2,edate IN varchar2)
AUTHID CURRENT_USER
IS
...
OPEN cdat FOR
     select
            ...
            '-', --NVL(TO_CHAR(trunc(c.P2,2)),'-'), -- 11 данные отсутствуют
            ...
            '-', --NVL(TO_CHAR(trunc(c.m4,2)),'-'),-- 15 M1гвс? данные отсутствуют?
            ...
            NVL(TO_CHAR(60-c.errtime),'-'), -- 23 Tp -- отсутствует в старых ТВ
            '-', --NVL(TO_CHAR(c.errtime),'-'), -- 24 Too -- отсутствует в старых ТВ
            '-', --NVL(TO_CHAR(c.errtime),'-'),  -- 25 Tog -- отсутствует в старых ТВ
...
--     ORDER BY "Дата" desc;
 ...
 utl_file.put_line(fd,'дд.мм.гггг'||chr(9)||'ч'||chr(9)||'т'||chr(9)||'м3'||chr(9)||'гр.C'||chr(9)||'кгс/см2'||chr(9)||'Гкал'||chr(9)||'т'||chr(9)||'м3'||chr(9)||'гр.C'||chr(9)||'кгс/см2'||chr(9)||'Гкал'||chr(9)||'т'||chr(9)||'Гкал'||chr(9)||'т'||chr(9)||'м3'||chr(9)||'гр.C'||chr(9)||'кгс/см2'||chr(9)||'Гкал'||chr(9)||'гр.C'||chr(9)||'кгс/см2'||chr(9)||'мин'||chr(9)||'мин'||chr(9)||'мин'||chr(9)||'мин'||chr(9)||'-');
     LOOP
         FETCH cdat INTO
