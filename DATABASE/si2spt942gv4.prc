create or replace procedure si2spt942gv4(p IN varchar2,sdate IN varchar2,edate IN varchar2)
AUTHID CURRENT_USER
IS
...
--            c.dcounter  as "Время",--2
            ...
            '-', --NVL(TO_CHAR(trunc(c.q2,6)),'-'), -- 12 W2 тепловая энергия 2016.04.05-добавил
            NVL(TO_CHAR((trunc(c.m1,6)-trunc(c.m2,6))),'-'),-- as "dM12", -- 13 dM12
            NVL(TO_CHAR(trunc(c.q1,6)),'-'), -- 14 dW тепловая энергия (W2 отсутствует, разница W1-W2=W1) добавил 07.04.2016
            -- '-', --NVL(TO_CHAR((trunc(c.q1,6)-trunc(c.q2,6))),'-'), -- as "dW12", -- 14 dW12 2016.04.05-добавил
            -- ГВС ---
