create or replace procedure si1spt943(p IN varchar2,sdate IN varchar2,edate IN varchar2)
AUTHID CURRENT_USER
IS
...
-- СИ-1 Дата,Время,M1,V1,t1,P1,W1,  M2,V2,t2,P2,W2,  dM,dW,tхв,Pхв,Tо,Tр,Tоо,НС
  ...
  cP1   varchar2(10);-- 6 datacurr.p1%type;
  cW1   varchar2(10);-- 7 datacurr.q1%type; -- Q --> W тепловая энергии
  cM2   varchar2(10);-- 8 datacurr.m2%type;
  cV2   varchar2(10);-- 9
  cT2   varchar2(10);-- 10 datacurr.T2%type;
  cP2   varchar2(10);-- 11
  cW2   varchar2(10);-- 12 datacurr.q2%type; -- Q --> W тепловая энергии
-- dM,dW,tхв,Pхв,Tо,Tр,Tоо,НС
  cdM   varchar2(10);-- 13 datacurr.dm12%type;
  cdW   varchar2(10);-- 14 datacurr.dg12%type;
--  cM1g  varchar2(10);-- datacurr.m3%type; --M1гвс m3 2009.04.15 OK!
...
--  cW1g  varchar2(10);-- datacurr.q3%type;
  cTxv  varchar2(10);-- 15 datacurr.tcool%type; --TCool -> tхв ??????????
  cPxv  varchar2(10);-- 16 datacurr.pxb%type; -- PXB -> Pхв ?????
  cT_o  varchar2(10);-- 17
  cTp   varchar2(10);-- 18
  cToo  varchar2(10);-- 19
--  cTog  varchar2(10);
--  cHC1   datacurr.Hc_1%type;
  cHC2   datacurr.Hc_2%type;-- 20
...
OPEN cdat FOR
     select
            ...
            NVL(TO_CHAR(trunc(c.P1)),'-'), -- 6 данные отсутствуют -- 20.06.2008 добавил данные
            NVL(TO_CHAR(trunc(c.q1,6)),'-'),  -- 7 W1 тепловая энергия
            -- ТВ2 - M2,V2,t2,P2,W2,
            NVL(TO_CHAR(trunc(c.M2,6)),'-'), -- 8
            NVL(TO_CHAR(trunc(V2,2)),'-'), -- 9 данные отсутствуют!!!!!!!!!!!!!!!!!!
            NVL(TO_CHAR(trunc(c.t2,6)),'-'), -- 10
            NVL(TO_CHAR(trunc(c.P2,6)),'-'), -- 11 данные отсутствуют -- 20.06.2008 добавил данные
            NVL(TO_CHAR(trunc(c.q2,6)),'-'),  -- 12 W2 тепловая энергия
            -- ГВС - dM,dW,tхв,Pхв,Tо,Tр,Tоо,НС
            NVL(TO_CHAR((trunc(c.m1,6)-trunc(c.m2,6))),'-'),-- as "dM12", -- 13 dM12
            NVL(TO_CHAR((trunc(c.q1,6)-trunc(c.q2,6))),'-'), -- as "dW12", -- 14 dW12
            --NVL(TO_CHAR(trunc(c.m3,6)),'-'),-- 15 M1гвс 2009.04.15 OK!
            --NVL(TO_CHAR(trunc(c.v4,6)),'-'), -- 16 V1гвс??????
            --'-',--NVL(TO_CHAR(trunc(c.t4,2)),'-'), -- 17 T1гвс данные отсутствуют!!!
            --'-', --NVL(TO_CHAR(trunc(c.p4,2)),'-'), -- 18 P1гвс ????????????
            --'-', --NVL(TO_CHAR(trunc(c.q4,2)),'-'), -- 19 W1гвс ????????????
            NVL(TO_CHAR(trunc(c.tcool,2)),'-'), -- 15 TCool -> tхв
            NVL(TO_CHAR(trunc(c.pxb,2)),'-'), -- 16 PXB -> Pxv ??????
--            NVL(TO_CHAR(c.errtime),'-'), -- 22 To
--            NVL(TO_CHAR(c.worktime),'-'), -- 23 Tp
            NVL(TO_CHAR(1-c.tsum1),'-'), -- 17 To (edited 2008.05.16)
            NVL(TO_CHAR((c.tsum1)),'-'), -- 18 Tp (edited 2008.05.16)
            NVL(TO_CHAR(c.errtime),'-'), -- 19 Too
            --NVL(TO_CHAR(c.errtime),'-'),  -- 25 Tog
--            SUBSTR(c.hc,1,16) -- для MT200,ТСР показывает коды
            NVL(TO_CHAR(c.hc_code),'-')-- 20
...
--     AND c.dcounter<=to_date(edate||' 23:59:59','DD.MM.YYYY HH24:MI:SS')
     AND c.dcounter<=(to_date(edate ,'DD.MM.YYYY') +1)
     ORDER BY "Дата","Время" desc;
--     ORDER BY "Дата" desc;
-- СИ-1 Дата,Время,M1,V1,t1,P1,W1,  M2,V2,t2,P2,W2,  dM,dW,tхв,Pхв,Tо,Tр,Tоо,НС
 utl_file.put_line(fd,'Дата'||chr(9)||'Время'||chr(9)||'M1'||chr(9)||'V1'||chr(9)||'t1'||chr(9)||'P1'||chr(9)||'W1'||chr(9)||'M2'||chr(9)||'V2'||chr(9)||'t2'||chr(9)||'P2'||chr(9)||'W2'||chr(9)||'dM'||chr(9)||'dW'||chr(9)||'tхв'||chr(9)||'Pхв'||chr(9)||'Tо'||chr(9)||'Tр'||chr(9)||'Tоо'||chr(9)||'НС');
 utl_file.put_line(fd,'дд.мм.гггг'||chr(9)||'ч'||chr(9)||'т'||chr(9)||'м3'||chr(9)||'гр.C'||chr(9)||'кгс/см2'||chr(9)||'Гкал'||chr(9)||'т'||chr(9)||'м3'||chr(9)||'гр.C'||chr(9)||'кгс/см2'||chr(9)||'Гкал'||chr(9)||'т'||chr(9)||'Гкал'||chr(9)||'гр.C'||chr(9)||'кгс/см2'||chr(9)||'ч'||chr(9)||'ч'||chr(9)||'ч'||chr(9)||'-');
     LOOP
         FETCH cdat INTO
               cdata,chour,cM1,cV1,cT1,cP1,cW1, --ТВ1
