create or replace procedure bami_si2vkt7(p IN varchar2,sdate IN varchar2,edate IN varchar2)
AUTHID CURRENT_USER
IS
nodename          varchar2(24);--:= 'Верейская 34-36' id_bd=297 26.12.2013;
--nodename = p;
...
--  chour datacurr.dcounter%type;
  ...
  cM1g  varchar2(10);-- 15 datacurr.m3%type; --M1гвс m3 2009.04.15 OK!
...
-- fd:= utl_file.fopen('D:\db\utl', nodename||'.txt', 'w'); -- домашний каталог
 ...
 utl_file.put_line(fd,'Код схемы измерений:'||chr(9)||'$$$2$$$'||kod_scheme); --kod_scheme
...
--            '-' as "V1", --NVL(TO_CHAR(trunc(c.v1,2)),'-'),           -- 4 V1 (ТВ1) не измеряется
            NVL(TO_CHAR(trunc(c.v1,6)),'-'),                            -- 4
            NVL(TO_CHAR(trunc(c.t1,6)),'-'),                            -- 5 t1 (ТВ1)
            NVL(TO_CHAR(trunc(c.P1,6)),'-'),                            -- 6 P1 (ТВ1) ? измеряется ?
            '-', -- Заменил                                             -- W1
--            NVL(TO_CHAR(trunc(c.q1,6)),'-'),                          -- 7 W1 (W1 и W2 не отображаются)
/* ТВ2 начало ------------------------------------------------------------------- */
            NVL(TO_CHAR(trunc(c.M2,6)),'-'),                            -- 8 V2 (ТВ1)
--            '-' as "V2", --NVL(TO_CHAR(trunc(V2,2)),'-'),             -- 9 V2 (ТВ1) не измеряется
            NVL(TO_CHAR(trunc(V2,6)),'-'),                              -- 9 V2 (ТВ1)
            NVL(TO_CHAR(trunc(c.t2,6)),'-'),                            -- 10 t2
            NVL(TO_CHAR(trunc(c.P2,6)),'-'),                            -- 11 P2
--            NVL(TO_CHAR(trunc(c.q2,6)),'-'),                          -- 12 W2 (W1 и W2 не отображаются)
            '-' as "W2", --NVL(TO_CHAR(trunc(c.q2,6)),'-') as "W2",     -- 12 W2 тепловая энергия не измеряется
            NVL(TO_CHAR((trunc(c.m1,6)-trunc(c.m2,6))),'-'),            -- 13 dM12
            NVL(TO_CHAR((trunc(c.q1,6)-trunc(c.q2,6))),'-'),            -- 14 dW12 (W1 и W2 не отображаются)
/* ГВС начало ------------------------------------------------------------------- */
            NVL(TO_CHAR(trunc(c.m4,6)),'-'),                            -- 15 M4 (ТВ2)
            NVL(TO_CHAR(trunc(c.v4,6)),'-'),                            -- 16 V4 (ТВ2)
            NVL(TO_CHAR(trunc(c.t4,6)),'-'),                            -- 17 T1гв (ТВ2)
            /*'-' as "P1гв",*/
            NVL(TO_CHAR(trunc(c.p4,2)),'-'),                            -- 18 P1гвс (ТВ2) p4
--            '-' as "W1гв", --NVL(TO_CHAR(trunc(c.q4,2)),'-'),         -- 19 W1гвс (ТВ2)
            NVL(TO_CHAR(trunc(c.q2,6)),'-'),                            -- 19 W1гвс (ТВ2)
            /* ТВ2 завершение --------------------------------------------------------------- */
           -- '-' as "tхв", --
            NVL(TO_CHAR(trunc(c.tcool,2)),'-'),                         -- 20 TCool -> tхв не измеряется
            --'-' as "Pхв", --
            NVL(TO_CHAR(trunc(c.pxb,2)),'-'),                           -- 21 PXB -> Pxv не измеряется
...
--            NVL(TO_CHAR((c.tsum1)),'-'),                              -- 23 Tp (edited 2008.05.16)
            NVL(TO_CHAR(c.errtime),'-'),                                -- 22 To (время останова)
            NVL(TO_CHAR(c.oktime),'-'),                                 -- 23 Tp (время работы)
            NVL(TO_CHAR(c.errtime),'-'),                                -- 24 Too (отключение отопления ТВ1)
            NVL(TO_CHAR(c.errtime2),'-'),                               -- 25 Tогв (отключения горячей воды ТВ2)
--            SUBSTR(c.hc,1,16) -- для MT200,ТСР показывает коды
            NVL(TO_CHAR(c.hc_code),'-')                                 -- 26 HC
/*код схемы СИ-2 (26) Дата, Время,M1,V1,t1,P1,W1,M2,V2,t2,P2,W2,dM,dW,M1гв,V1гв,t1гв,P1гв,W1гв,tхв,Pхв,To,Tp,Too,Tогв,НС */
     from bdevices d, datacurr c, bbuildings b
     where c.id_bd = d.id_bd AND d.id_bu = b.id_bu AND c.id_ptype=3
--     AND b.cshort = 'АТС 272-2'--'АТС 271', 'АТС 272-2'
     AND b.cshort = nodename --'АТС 750'
...
--     AND c.dcounter<=to_date('27.12.2012 23:00:00','DD.MM.YYYY HH24:MI:SS')
     ...
     AND c.dcounter<=to_date(edate,'DD.MM.YYYY HH24:MI:SS')--+(60/1440)
