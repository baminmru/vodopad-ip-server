create or replace procedure si6spt942(p IN varchar2,sdate IN varchar2, edate IN varchar2)
AUTHID CURRENT_USER
IS
nodename          varchar2(24);--:= 'СММТ';
--nodename = p;
...
OPEN cdat FOR
     select
            ...
            NVL(TO_CHAR(trunc(c.q1,6)),'-'),  -- 7 W1-ТСР; Q1-942 тепловая энергия
...
/*код схемы СИ-6 Дата,Время,M1,V1,t1,P1,W1з,M2,t2,P2,To,Tp,Too,НС */
...
--     ORDER BY "Дата" desc;
 utl_file.put_line(fd,'Дата'||chr(9)||'Время'||chr(9)||'M1'||chr(9)||'V1'||chr(9)||'t1'||chr(9)||'P1'||chr(9)||'W1з'||chr(9)||'M2'||chr(9)||'t2'||chr(9)||'P2'||chr(9)||'Tо'||chr(9)||'Tр'||chr(9)||'Tоо'||chr(9)||'НС');
 utl_file.put_line(fd,'дд.мм.гггг'||chr(9)||'ч'||chr(9)||'т'||chr(9)||'м3'||chr(9)||'гр.C'||chr(9)||'кгс/см2'||chr(9)||'Гкал'||chr(9)||'т'||chr(9)||'гр.C'||chr(9)||'кгс/см2'||chr(9)||'ч'||chr(9)||'ч'||chr(9)||'ч'||chr(9)||'-');
     LOOP
         FETCH cdat INTO
