...
namespace STKService
{
    public partial class STKService 
    {
        ...
        private int ActiveThreadsCount()
        {
            ...
            {
                tt = pair.Value;
                if (tt.Process.HasExited == true || tt.Process.StartTime.AddMinutes(2) <= DateTime.Now)
                {
                    try
                    {
                        tt.Process.Kill();
                    ...
                    try
                    {
                        ...
                        if (oRS != null)
                        {
                            ...
                            if (oRS.Rows.Count > 0)
                            {
                                ...
                                for (int i = 0; i < oRS.Rows.Count && i < 30 && ActiveThreadsCount() <= ModemCount + 1; i++)
                                {
                                   ...
                                   Thread.Sleep(1500);  // wait for modem pull locking
                                }
