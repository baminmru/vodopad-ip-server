...
lbl_pass2:
                ...
                If True Then
                    ...
                    If OK Then
                        ...
                        sfile = SZTDB.ExecTGK1Proc(dt.Rows(i)("id_bd"), "BAMI_" & dt.Rows(i)("proc_prefix") & dt.Rows(i)("proc_postfix"), dt.Rows(i)("CSHORT"), sdate, edate)
                        ...
                        If sfile <> "" Then
                            ...
                            If OK Then
                                ...
                                End Try
                            Else
                                My.Computer.FileSystem.WriteAllText(LogPath(), "нет VPN соединения  " & vbCrLf, True, System.Text.Encoding.Default)
...
storefile:
                        ...
                        Else
                            My.Computer.FileSystem.WriteAllText(LogPath(), "Error[no file]- " & dt.Rows(i)("proc_prefix") & dt.Rows(i)("proc_postfix") & " " & dt.Rows(i)("CSHORT") & vbCrLf, True, System.Text.Encoding.Default)
                            Log = Log & "Error[No file to send]- " & dt.Rows(i)("proc_prefix") & dt.Rows(i)("proc_postfix") & " " & dt.Rows(i)("CSHORT")
                        End If
                    Else
    ...
    '    Dim fileData As String
    '    dt = SZTDB.GetTGK1Dev()
    '    dtm = SZTDB.GetMISSING()
    '    For i = 0 To dtm.Rows.Count - 1
    '        adat = dtm.Rows(i)("archdate")
    '        'adat = DateAdd(DateInterval.Hour, -1, adat)
    '        id_bd = dtm.Rows(i)("id_bd")
    '        For j = 0 To dt.Rows.Count - 1
    '            If id_bd = dt.Rows(j)("id_bd") Then
    '                Exit For
    '            End If

    '        Next
    '        If SZTDB.CheckForArch(3, adat.Year, adat.Month, adat.Day, adat.Hour, id_bd) Then
    '            'build file

    '            'сформировать архив по узлу
    '            sfile = SZTDB.ExecTGK1Proc(id_bd, "BAMI_" & dt.Rows(j)("proc_prefix") & dt.Rows(j)("proc_postfix"), dt.Rows(j)("CSHORT"), DateAdd(DateInterval.Minute, -2, adat), DateAdd(DateInterval.Minute, +2, adat))
    ...
    '                'перекладываем файл в директорию с  неотправленными файлами
    '                My.Computer.FileSystem.WriteAllText(System.IO.Path.GetDirectoryName(Me.GetType().Assembly.Location()) + "\FilesToSend\" & Guid.NewGuid.ToString() & ".dat", fileData, False, System.Text.Encoding.Default)
    ...
    '                'delete row from missingarch
    '                ' SZTDB.DeleteMISSING(id_bd, adat)
