...
Public Class frmControl
    ...
    Private Sub LoadIP()
        Dim xlist As XmlDocument
        xlist = New XmlDocument
        xlist.Load(System.IO.Path.GetDirectoryName(Application.ExecutablePath) & "\ip.xml")
        ...
        Dim dr As DataRow
        For Each xnode In xlist.LastChild.ChildNodes
            dr = dt.NewRow
            dr("Name") = xnode.Attributes.GetNamedItem("Name").Value + " (" + xnode.Attributes.GetNamedItem("Address").Value + ")"
            dr("Address") = xnode.Attributes.GetNamedItem("Address").Value
            dt.Rows.Add(dr)
        Next

        'dt = QuerySelect("select CSHORT,NPIP from  bbuildings b join bdevices d on b.id_bu= d.id_bu where transport=3")

        txtIP.DisplayMember = "Name"
        txtIP.ValueMember = "Address"
        txtIP.DataSource = dt
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        If f.DialogResult = DialogResult.OK Then
            ...
            xlist = New XmlDocument
            xlist.Load(System.IO.Path.GetDirectoryName(Application.ExecutablePath) & "\ip.xml")
            Dim xIP As XmlElement = xlist.CreateElement("IP")
            Dim xmlAttr As XmlAttribute
            xmlAttr = xlist.CreateAttribute("Name")
            xmlAttr.Value = f.txtName.Text
            xIP.Attributes.Append(xmlAttr)

            xmlAttr = xlist.CreateAttribute("Address")
            xmlAttr.Value = f.txtAddress.Text
            xIP.Attributes.Append(xmlAttr)

            xlist.LastChild.AppendChild(xIP)

            xlist.Save(System.IO.Path.GetDirectoryName(Application.ExecutablePath) & "\ip.xml")
            LoadIP()
    ...
    Private Sub Button1_ControlRemoved(ByVal sender As Object, ByVal e As System.Windows.Forms.ControlEventArgs) Handles Button1.ControlRemoved
