...
Public Class frmSetupGrid
    ...
    Public Sub LoadData()
        ...
        Select Case ptype
            Case 1
                sqlstr = "select ml.* from bdevices b left join masksline ml on b.id_mask_curr= ml.id_mask  where b.id_bd = :id order by sequence"

            Case 3
                sqlstr = "select ml.* from bdevices b left join masksline ml on b.id_mask_hour= ml.id_mask  where b.id_bd = :id order by sequence"

            Case 4
                sqlstr = "select ml.* from bdevices b left join masksline ml on b.id_mask_24= ml.id_mask  where b.id_bd = :id order by sequence"

            Case 2
                sqlstr = "select ml.* from bdevices b left join masksline ml on b.id_mask_sum= ml.id_mask  where b.id_bd = :id order by sequence"
        End Select
        ...
        For i = 0 To Grid.Columns.Count - 1
            column = Grid.Columns.Item(i)

            r = cdt.NewRow
            r("CFLD") = column.Name
            r("CHEADER") = column.HeaderText
            r("SEQUENCE") = column.DisplayIndex
            r("COLWIDTH") = column.Width
            r("COLHIDDEN") = 0
            r("COLFORMAT") = "?"
            cdt.Rows.Add(r)
        ...
        Select Case ptype
            Case 1
                sqlstr = "select ml.* from bdevices b join masks ml on b.id_mask_curr= ml.id_mask  where b.id_bd = :id"

            Case 3
                sqlstr = "select ml.* from bdevices b join masks ml on b.id_mask_hour= ml.id_mask  where b.id_bd = :id"

            Case 4
                sqlstr = "select ml.* from bdevices b join masks ml on b.id_mask_24= ml.id_mask  where b.id_bd = :id"

            Case 2
                sqlstr = "select ml.* from bdevices b join masks ml on b.id_mask_sum= ml.id_mask  where b.id_bd = :id"
        End Select
        cmd.CommandText = sqlstr
        ...
        If dt2.Rows.Count > 0 Then
            txtName.Text = dt2.Rows(0)("CNAME")
            mask_id = dt2.Rows(0)("ID_MASK")
        End If
    ...
    Private Function GetWidth(ByVal key As String) As Integer
        ...
        With Grid
            Try
                sz = .Columns.Item(key).MinimumWidth
            Catch ex As Exception
                sz = -1
            End Try
    ...
    Private Sub frmSetupGrid_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
