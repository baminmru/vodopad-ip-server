   ...
   ByVal ArchMonth As Int32, ByVal ArchDay As Int32, ByVal ArchHour As Int32) As String
    ...
    Public Function GetNextModem(ByVal TYPECALL As String) As String
        ...
        If TYPECALL.ToUpper = "G" Then
            query = "select ID_CP,CIPADDR,TERMINAL,MACHINE,COMPORT,CTYPECALL ,useduntil from ipaddr join comports on ipaddr.id_ip=comports.id_ip  where CTYPECALL='G' and  ( UsedUntil is null or UsedUntil < sysdate) and TERMINAL='" & MyName & "'" + "  order by COMPORT"
        Else
            query = "select ID_CP,CIPADDR,TERMINAL,MACHINE,COMPORT,CTYPECALL ,useduntil from ipaddr join comports on ipaddr.id_ip=comports.id_ip  where CTYPECALL<>'G' and  ( UsedUntil is null or UsedUntil < sysdate) and TERMINAL='" & MyName & "'" + "  order by COMPORT"
        End If
    ...
    Public Sub FreeModem()
        Dim query As String
        If Mymodem > 0 Then
            query = "update comports set useduntil=sysdate-20/60/24 where id_cp=" & Mymodem.ToString
            QueryExec(query)
