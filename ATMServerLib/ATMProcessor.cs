            ...
            if (bLogged)
            {
                ...
                else
                {
                    if (devRS.Rows.Count ==0)
                        ErrorReport("Не обнаружен прибор для ATM устройства с идентификатором: " + aSocket.callerID);
                    else
                        ErrorReport("Обнаружено " + devRS.Rows.Count.ToString() + " приборов для ATM устройства с идентификатором: " + aSocket.callerID);
                }
            }
