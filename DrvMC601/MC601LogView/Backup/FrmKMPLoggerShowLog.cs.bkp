  ...
  public class FrmKMPLoggerShowLog : Form
  {
    ...
    internal void DisplayData()
    {
      ...
      foreach (KMPLogDataSet.RegistersRow registersRow in this.KMPDS.Registers)
      {
        ...
        row["Id"] = (object) registersRow.Id;
        row["TimeStamp"] = (object) registersRow.Date;
        if (registerInUseRow.INFO)
        {
          row["Info Code"] = (object) registersRow.INFO;
          row["Info"] = (object) ClsUtils.InfoCodeToString(Convert.ToDouble(registersRow.INFO));
        }
        if (registerInUseRow.INFO)
        {
          row["LogQOS Code"] = (object) registersRow.LogQOS;
          row["LogQOS"] = (object) ClsUtils.LogQOSCodeToString(registersRow.LogQOS);
        }
