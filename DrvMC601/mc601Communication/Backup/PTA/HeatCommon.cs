  ...
  internal class HeatCommon
  {
    public static void Trace(TraceLevel traceLevel, string Appl, string Message)
    {
      ...
      if (traceLevel == TraceLevel.Error)
        Trace.WriteLineIf((traceSwitch.TraceError ? 1 : 0) != 0, str + "  " + Appl + " --- " + Message);
      else if (traceLevel == TraceLevel.Warning)
        Trace.WriteLineIf((traceSwitch.TraceWarning ? 1 : 0) != 0, str + "  " + Appl + " --- " + Message);
      else if (traceLevel == TraceLevel.Info)
      {
        Trace.WriteLineIf((traceSwitch.TraceInfo ? 1 : 0) != 0, str + "  " + Appl + " --- " + Message);
      }
      else
      {
        if (traceLevel != TraceLevel.Verbose)
          return;
        Trace.WriteLineIf((traceSwitch.TraceVerbose ? 1 : 0) != 0, str + "  " + Appl + " --- " + Message);
      }
    }
    ...
    public static void Trace(TraceLevel traceLevel, string Appl, string sFunction, string ExceptionMessage, string StackTrace)
    {
      ...
      if (traceLevel == TraceLevel.Error)
        Trace.WriteLineIf((traceSwitch.TraceError ? 1 : 0) != 0, str + "  " + Appl + "-Function:" + sFunction + "\r\n            Ex:" + ExceptionMessage + "\r\n            ST:" + StackTrace);
      else if (traceLevel == TraceLevel.Warning)
        Trace.WriteLineIf((traceSwitch.TraceWarning ? 1 : 0) != 0, str + "  " + Appl + "-Function:" + sFunction + "\r\n            Ex:" + ExceptionMessage + "\r\n            ST:" + StackTrace);
      else if (traceLevel == TraceLevel.Info)
      {
        Trace.WriteLineIf((traceSwitch.TraceInfo ? 1 : 0) != 0, str + "  " + Appl + "-Function:" + sFunction + "\r\n            Ex:" + ExceptionMessage + "\r\n            ST:" + StackTrace);
      }
      else
      {
        if (traceLevel != TraceLevel.Verbose)
          return;
        Trace.WriteLineIf((traceSwitch.TraceVerbose ? 1 : 0) != 0, str + "  " + Appl + "-Function:" + sFunction + "\r\n            Ex:" + ExceptionMessage + "\r\n            ST:" + StackTrace);
      }
    }
  }
}
