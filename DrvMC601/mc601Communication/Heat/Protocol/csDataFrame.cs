...
namespace Kamstrup.Heat.Protocol
{
  public class csDataFrame
  {
    ...
    public csDataFrame(byte bytStart, byte bytDestinationAddress, byte bytCID, ArrayList arrData, byte bytStop)
    {
      ...
      this.m_arrCommBuffer.Add( bytStart);
      this.m_bytDestinationAddress = bytDestinationAddress;
      this.m_arrCommBuffer.Add( bytDestinationAddress);
      this.m_bytCID = bytCID;
      this.m_arrCommBuffer.Add( bytCID);
      if (arrData.Count > 0)
    ...
    private ArrayList Unstuff(ArrayList arrCommBuffer)
    {
      ...
      for (int index = 0; index < arrayList2.Count; ++index)
        arrayList2[index] =  ~(byte) arrayList2[index];
      arrayList2.Sort();
      int index1 = -1;
      try
      {
          foreach (byte num in arrCommBuffer)
          {
              if ((int)num == (int)ControlCharacters.Stuff)
                  flag = true;
              else if (flag && (index1 = arrayList2.BinarySearch(num)) >= 0)
              {
                  arrayList1.Add(~(byte)arrayList2[index1]);
                  flag = false;
              }
              else
                  arrayList1.Add(num);
    ...
    private ArrayList Stuff(ArrayList arrCommBuffer)
    {
      ...
      {
        int index;
        if ((index = arrayList2.BinarySearch( num)) >= 0)
        {
          arrayList1.Add( 27);
          arrayList1.Add( ~(byte) arrayList2[index]);
        }
        else
          arrayList1.Add( num);
      }
